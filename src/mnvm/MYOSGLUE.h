...
#define MyMoveBytes(src,dst,count) memcpy(dst, src, count)
...
/* more glue for obj-c */
GLOBALPROC DiskEjectedNotify(ui4b Drive_No);
GLOBALPROC DiskInsertNotify(ui4b Drive_No, blnr locked);
GLOBALFUNC blnr FirstFreeDisk(ui4b *Drive_No);
GLOBALFUNC ui5b Date2MacSeconds(int second, int minute, int hour,
								int day, int month, int year);
GLOBALFUNC blnr InitEmulation(void);
GLOBALPROC Screen_Draw(si3b TimeAdjust);
